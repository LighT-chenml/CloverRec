...
RESERVED_LEN = 16  # We should reserve some space for receiving because the length of exchanged strings may be not equal
...
class CM(CommBase):
    def __init__(self, port, ip=None):
        ...
        self.recv_mr = self.cmid.reg_msgs(1024 + RESERVED_LEN)
        self.send_mr = self.cmid.reg_msgs(1024)
    ...
    def handshake(self, **kwargs):
        ...
        try:
            ...
            self.cmid.post_send(self.send_mr, 0, size)

            recv_wc = self.cmid.get_recv_comp()
            send_wc = self.cmid.get_send_comp()

            recv_msg = self.recv_mr.read(recv_wc.byte_len, 0).decode('utf-8')
