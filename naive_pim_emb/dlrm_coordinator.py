...
def run():
    ...
    with torch.autograd.profiler.profile(
        ...
        print("Avg latency (PIM + EV_trans + CPU + GPU + network + others) (ms) : " + str(round(avg_latency, 2)))
        avg_ev_lookup_time = sum(dlrm.ev_lookup_time) / len(dlrm.ev_lookup_time)
        print("Avg ev lookup time (PIM + EV_trans + CPU) (ms) : " + str(round(avg_ev_lookup_time, 2)))
        avg_apply_emb_time = sum(dlrm.apply_emb_time) / len(dlrm.apply_emb_time)
        print("Avg apply emb time (PIM + EV_trans + CPU + GPU + network) (ms) : " + str(round(avg_apply_emb_time, 2)))
       
        avg_CPU_cal_time = sum(dlrm.emb_storage.CPU_cal_time) / len(dlrm.emb_storage.CPU_cal_time)
        print("CPU cal time (ms) : " + str(round(avg_CPU_cal_time, 2)))
        print("(PIM + EV_trans) time (ms) : " + str(round(avg_ev_lookup_time - avg_CPU_cal_time, 2)))
